...
window.scrollTo(0, 0); // Hamburger Nav// 
...
var iconbottom = document.querySelector('.navbar__hamburger-bottom'); // On Button//

var OnButton = document.querySelectorAll("a")[5]; // Opacity change function 
...
OnButton.addEventListener("click", function () {
  ...
  if (toggle === true) {
    OnButton.classList.add("on"); // // Section to show Media Player
    // if (toggle === true){
    // OnButton.classList.add("on");
    // Loop the animation

    interval = setInterval(draw, 33); // document.getElementsById("picchange").style.opacity = 1;
  } else {
    clearInterval(interval);
    OnButton.classList.remove("on");
    ctx.clearRect(0, 0, canvas.width, canvas.height); //  Add hide media player instruction here
